...
export const periodRange=(period:'today'|'week'|'month'|'year')=>{const end=new Date();const start=new Date();start.setHours(0,0,0,0);if(period==='week')start.setDate(start.getDate()-6);if(period==='month')start.setDate(1);if(period==='year'){start.setMonth(0);start.setDate(1)}return[start.toISOString(),end.toISOString()] as const}
